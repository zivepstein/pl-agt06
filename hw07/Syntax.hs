...
atom = try binopExpParser <|> try lam <|> try var <|> try ifParser <|> try letExpParser 
       <|> try unopExprParser <|> try pairParser <|> try assignParser <|> try letRecParser <|> try boolParser <|> try intParser <|> parens expr 
lam = do
  ids <- try (kw "lambda" *> space *> (try (parens tidentifier) `sepBy1` ws))  <|> kw "lambda" *> space *> ( try tidentifier `sepBy1` ws)   
  body <- (symbol "." *> expr)
...
binopExpParser, binopMul, binopFactor :: Stream s m Char => ParsecT s () m Expr
binopExpParser =  binopMul `chainl1` binopParserAnd -- <|> binopMul
binopMul = binopFactor `chainl1` binopParserMul -- <|> binopFactor
binopFactor = try (parens binopExpParser) <|> try intParser <|> boolParser
--(symbol "+" pure (BinopExp Plus))
 -- (\e1 bi e2 -> BinopExp bi e1 e2) <$> expr <*> binopParserAnd <*> expr

binopParserAnd, binopParserMul  :: Stream s m Char => ParsecT s () m (Expr -> Expr -> Expr)
binopParserAnd = try (return (BinopExp Plus) <* symbol "+") <|> try (return  (BinopExp Minus) <* symbol "-")
               <|> try (return (BinopExp Or) <* symbol "||") <|> try (return ( BinopExp Equals) <* symbol "==")
binopParserMul =  try (do{ symbol "*"; return (BinopExp Times)   })
         <|> try (do{ symbol "/"; return (BinopExp Div) })  <|> try (do{ symbol "&&"; return (BinopExp And) })
