...
-- > mainAux _ = die "you fucked up idiot"
...
mapReduceM m r input = do{ mapppy <- m; reduccy <- r; return ((\red -> Map.toList . Map.mapWithKey red) reduccy . ( Map.fromListWith (++)) . concatMap (map (\(k,v) -> (k,[v])) . mapppy) input)}

> listifyVal (k,v) = (k,[v])

> mapReduceM :: (Ord k, Monad m) => MapperM m a k v -> ReducerM m k v -> [a] -> m [(k,[v])]
> mapReduceM m r input = do{ mappy <- map m input; return $ map listifyVal mappy}
